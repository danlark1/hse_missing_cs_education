...
или компиляторы. Системы сборки &mdash; это, в какой-то степени, тоже программирование.
...
других, граф начинает исполняться, и Вы радостно смотрите, как проходит сборка в
...
знать, что происходит для сбора пакетов на низких уровнях, а на следующих лекциях мы
...
надо собирать всё, потратите лишнее время:
...
это искоренить. Основные потребители `make` &mdash; это, скорее, C репозитории. Стоит
помнить, что в `make` негласно придерживаются следующих опций для сборки C
кода, например:
...
который поможет собирать достаточно сложные проекты. Почти любой уважающий
...
Google выпустила свою систему сборки [ninja](https://ninja-build.org/),
...
Чем CMake так хорош? (ладно, на самом деле плох, но я постараюсь как-то в хорошем
...
Перед `<visibility>` можно написать `SYSTEM` &mdash; означает, что к хедерам
...
CMake уже больше похож на artifact-based: то есть даётся от силы сотня правил
...
Вообще, в индустрии очень любят нагородить фреймворков друг на друга,
...
С багами при обновлении помогут автоматические тесты, которые тестируют (в идеале)
...
Вы будете встречать на практике понятие CD (continuous delivery), это
считайте то же самое, только оно обычно при всех "зелёных тестах" создаёт
