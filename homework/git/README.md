...
делайте это в этот раз с помощью консольного редактора (vim, nano).
...
предназначено это рассуждение. Используйте консольный редактор для этого: vim, nano подойдут. Или пишите объяснение прям в командной строке. Пример:

```console
$ # Это комментарий для bash, можно нажимать enter и ничего не будет
```
...
каст. Также существует флаг `-i <sec>`, с ним `asciinema` будет обрезать фрагменты,
в которые на записи ничего не меняется, до `<sec>` секунд (`sec` может быть десятичной дробью).
Флаг `-i` можно использовать и для записи с тем же эффектом.
