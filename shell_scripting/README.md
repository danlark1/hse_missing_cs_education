...
В этой лекции мы расскажем о том, как писать скрипты, о том какие подъязыки
...
Общий синтаксис для `if` советуют придерживаться в двойных `[[]]` скобках
...
В регулярных выражениях поддерживаются стандартных `*, +, ?, {n,m}, \w, \s, [:alpha:]` и т.д.
...
grep очень удобен для pipe поиска, например, достаточно часто используется как
...
sed (**s**tream **ed**itor), это утилита для запуска скриптов, которые как-то меняют
