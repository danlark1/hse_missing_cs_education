...
так или иначе всплывает в работе инженера на системах Linux и MacOS при
разработке (извините, Windows почти не затронем), чтении и написании статьей или
...
автоматы, но критериев по этому нет и не будет, присылайте pull requests.

Многие задачи потребуют github репозитория. У вас две недели с начала этой
лекции, чтобы создать отдельный **закрытый** репозиторий, поделиться с `danlark1@`
и прислать его по этой TODO форме. Если вы это не сделаете это за 2 недели,
я поставлю вам ноль :)
...
в конце `.bashrc` добавить команду `exec zsh` или `exec fish`.

## tmux

[tmux](https://github.com/tmux/tmux/wiki) -- отдельная утилита для шелла,
которая расчитана на мультиработу и хранение состояний терминало. Например,
