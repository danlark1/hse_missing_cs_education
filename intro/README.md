...
знаю всё, я до сих пор учусь повышать свою инженерную продуктивность &ndash; этот
...
будет посвещена темам, которые интересны Вам, если Вы хотите, чтобы я покрыл
...
Единственные преимущества быть на лекции &ndash; возможность задавать вопросы лично и
...
достаточно ограничены в своих действиях &ndash; мы пока не умеем голосом объяснять
...
поговорим о самом популярном shell &ndash; Bourne Again SHell, или "bash" как его
...
где Вы находитесь. Первый `danlark` &ndash; это кто я, второй `danlark` &ndash;
машина на которой я нахожусь. `~` &ndash; стандартное обозначение `home` директории
...
`$PATH` &ndash; сам bash раскрывает эту переменную при исполнении.
...
С аргументами в shell есть сложности &ndash; основной разделитель является пробелом,
...
Путь, начинающийся с `/` &ndash; абсолютный путь, иначе он относительный. `.` это
текущая директория, `..` &ndash; родительская директория (предыдущая до последнего
...
* / search string &ndash; ищёт “search string” в текущем man
* n &ndash; к следующему совпадению строки
* shift + n &ndash; предыдущее совпадение строки
...
Иногда Вы можете встретить команду `&`, она делает detach процесса &ndash; это
...
Мой совет &ndash; откажитесь от обычного bash и используйте zsh или fish. Чтобы
...
[tmux](https://github.com/tmux/tmux/wiki) &ndash; отдельная утилита для шелла,
...
В tmux можно создавать сессии, в которых Вы работаете, сессии состоят из окон &ndash;
это наборы терминалов, а окна из панелей &ndash; это отдельные обычные терминалы.
Вот некоторые шорткаты, как работать с панелями

* `Ctrl+b "` &ndash; создать новую панель горизонтально от выбранной
* `Ctrl+b %` &ndash; создать новую панель вертикально от выбранной
* `Ctrl+b arrow key` &ndash; смена панели
* `Ctrl+b c` &ndash; создание нового окна
* `Ctrl+b n` &ndash; переключение на следующее окно
* `Ctrl+b p` &ndash; переключение на предыдудщее окно
* `Ctrl+b w` &ndash; показ всех сессий и окон и интерфейс для переключение
* `Ctrl+b ?` &ndash; показ всех команд
* `Ctrl+d, exit` &ndash; выход из панели, возвращение на предыдущую панель
...
* `cd`, `mkdir`, `touch`, `rm`, `touch`, `pwd`, `ls` &ndash; см. выше
* `sort`, `uniq` &ndash; сортировка и вывод уникальных
  (вход должен быть отсортированным)
* `grep`, `cut` &ndash; поиск по регулярному выражению (см. след лекцию) и сплит строк
* `head`, `tail` &ndash; вывод первых и последних строк
* `wc` &ndash; подсчёт количества байт, строк в файле итд
* `df` &ndash; статистика использованного места на машине
* `cat`, `tac` &ndash; вывод файла с начала или с конца соответственно
* `echo` &ndash; повторение аргументов
* `find` &ndash; мощная команда для поиска в директории
* `chmod`, `chown`, `sudo` &ndash; изменение permissions и овнеров, работа в режиме super user
