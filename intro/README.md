...
За время моей учёбы и работы я накопил достаточно большой багаж знаний. В этом мне помогали друзья, индустрия, [StackOverflow](http://letmegooglethat.com/?q=StackOverflow), но, кажется, на ПМИ подобного сугубо прикладного курса пока не было. Поэтому я в роли лектора решил взять на себя смелость рассказать об инструментах, идеях и возможностях, доступных и применяющихся в современной IT-индустрии и восполнить "пропущенный семестр компьютерной науки".

Я не утверждаю, что знаю всё &mdash; знать всё невозможно, да и незачем. В наше время часто появляются новые инструменты, не реже обновляются и старые, благодаря чему каждый может найти то, что ему по вкусу и со временем составить свой идеальный набор.

Поэтому у меня нет задачи заставить Вас использовать что-то, что использую я. Моя цель &mdash;
ввести Вас в волшебный мир инженерных фреймворков, инструментов и программ, и сделать так, чтобы "компьютерная магия" превратилась в простой "уличный фокус".

## Зачем нужен этот курс?

Мы знаем, что компьютеры отлично справляются с вычислительными
задачами. Они предоставляют широкий спектр инструментов, позволяющих нам быть
продуктивными и решать сложные задачи в любых областях.

Однако многие используют лишь малую часть доступных возможностей и, когда работа с системой заходит в тупик, в лучшем случае слепо копируют команды из [StackOverflow](http://letmegooglethat.com/?q=StackOverflow), не особенно разбираясь в том, как работают заклинания вроде `apt`, `vim`, `git` или `docker`.

Этот курс проводится для того, чтобы рассказать об устройстве популярных инструментов и фреймворков и показать, как их использовать, ведь умение правильной работы с, казалось бы, привычными программами &mdash; залог продуктивности!

## Структура

Курс содержит от 10 до 12 лекций, каждая из которых затрагивает определённую тему.
...
(извините, Windows почти не затронем), чтении и написании статей или
документации, при запуске сервисов, виртуализации,
тестировании, сборке и воспроизведении результатов других разработчиков. 

Последняя лекция
будет посвящена темам, которые интересны Вам. Если Вы хотите, чтобы я покрыл
определённую тему на последней лекции, заполните эту
[форму](https://forms.gle/EZCdUxjXuvozwA726). Последняя лекция будет полностью в записи и без
конспекта.

### Оценивание

Будет 3 домашних задания. Вот примерный список тем:

- Shell scripting
- gdb coredumps debug
- Опубликовать своё резюме в LaTeX через github actions

Финальная оценка будет ставиться как среднее всех домашних заданий. Экзамена не будет, семинаров тоже, обязательного посещения тоже нет.
Домашние задания не округляются; берутся дробные оценки в зависимости от
...
что в этом курсе нужно многое показывать. Если несколько человек найдут достаточное количество
...
Также подписывайтесь на [телеграм канал](https://t.me/hse_msemester_2020), он
является основной точкой получения информации. Чата
