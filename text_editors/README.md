...
5. Текстовые редакторы могут дать Вам 90+% функциональности IDE и быть в 10
   раз быстрее &mdash; поверьте, быстрая навигация, поиск, замена текста,
...
кодом удалённо или не могу сохранить код локально. У Vim богатая история &mdash; он был
...
* `da'` &mdash; удалить всё внутри одинарной кавычки и её в том числе
...
Так как vim это всегда про discoverability, нежели про написание кода, то
...
соответственно
* `Ctrl+w _` &mdash; установка высоты, число пишется перез <C-w>
* `Ctrl+w |` &mdash; установка ширины, число пишется перез <C-w>
...
Это ещё одна очень сильная точки для Vim &mdash; очень много плагинов, под разные
...
* `Ctrl+/` &mdash; комментирование выделенных строк в зависимости от языка и раскоментирование, если оно закоментировано
...
2. Программисты (как я), которые пишут прототип без понимания явных гарантий
  всех функций. У них workflow устроен так &mdash; написать кода, посмотреть что не
