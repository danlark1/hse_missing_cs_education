...
так, чтобы другие комьютеры нас не смогли взломать. Из хороших
новостей, с которыми Вы скорее всего как-то сталкивались на алгоритмах:
...
тактику, которая указана в комиксе, генерация 4-5 слов, каждое привносит
...
поймёт, о чём. Предотвращает атаки MITM или Meet In The Middle в терминах
безопасности, если Вы смогли публичный ключ доверительно отослать (например,
Ваш провайдер может подменять все сайты :-)).
...
ключа, а другие его могут раскодировать Вашим публичным и наоборот &mdash; все
...
которым доверяют. И дальше они спокойно кодируют сообщения с Вашими ключами на
Вашей машине.
...
Похожим образом работают транзакци в банковской системе, но мы оставляем это
за пределами нашего обсуждения.

Так работает end-to-end с некоторыми наворотами, которые позволяют создавать
общий секрет только между двумя людьми с алгоритмом [Diffie-Hellman](https://en.wikipedia.org/wiki/Diffie%E2%80%93Hellman_key_exchange). Так отчасти работают чаты в Телеграм и
совпадение картинки гарантирует, что канал установлен и никто не слушает Вас.
Конечно, если картинка отражает однозначно общий ключ. Чтобы избежать хранения
...
Android линейки только Pixel линейка может похвастаться такой же скорость.
...
она просит дать телефон другу, чтобы подтвердить, что это Вы". В интернете
это в основном телефон или [YubiKey](https://www.yubico.com/).

Используйте её только с [Auth apps](https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2&hl=en_GB&gl=US), которые
